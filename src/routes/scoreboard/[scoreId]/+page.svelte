<script lang="ts">
    ...
    const userScore = axios.get(`${data.baseURL}/api/scores/${data.scoreId}`).then((res) => res.data.data); 
    const allScores = axios.get(`${data.baseURL}/api/scores`).then((res) => res.data.data);
...
{:then scores}
    {#if Object.keys(scores[0]).length > 0}
